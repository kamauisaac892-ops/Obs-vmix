...
function embedFor(which,item){
  const f=$(which==='program'?'programEmbed':'previewEmbed'),src=embedSrc(item);
  if(!f||!src)return;
  if(f.dataset.src!==src){f.dataset.src=src;f.src=src;}
f.hidden=false;f.style.display='block';f.style.position='absolute';f.style.left='0';f.style.top='0';f.style.width='100%';f.style.height='100%';f.style.zIndex='15';
  f.onload=()=>{toast((item.provider||'Media').toUpperCase()+' player loaded')};
  f.onerror=()=>toast('The external video player could not be embedded. The owner may have disabled embedding.');
}
...
async function capture(source){
  if(state.sources.has(source))return state.sources.get(source);
  if(!window.isSecureContext || !navigator.mediaDevices){
    toast('Camera and screen require the HTTPS version of the site');
    return null;
  }
  try{
    const stream=source==='screen'
      ? await navigator.mediaDevices.getDisplayMedia({video:true,audio:true})
      : await navigator.mediaDevices.getUserMedia({video:{facingMode:source==='camera1'?'environment':'user',width:{ideal:1920},height:{ideal:1080}},audio:false});const v=document.createElement('video');v.srcObject=stream;v.autoplay=true;v.playsInline=true;v.muted=true;await v.play().catch(()=>{});videos.set(source,v);const item={id:source,name:source==='screen'?'Screen share':source==='camera1'?'Camera 1':'Camera 2',kind:'stream',stream};state.sources.set(source,item);return item
  }catch(e){
    const name=e?.name||'UnknownError';
    if(name==='NotAllowedError')toast('Camera/screen permission is blocked. Allow it for obs-vmix.vercel.app in the browser address-bar permissions.');
    else if(name==='NotFoundError')toast('No camera or screen capture device is available.');
    else toast('Camera/screen could not start: '+name);
    return null
  }
}
...
async function selectSource(source){
  if(source==='media'){ $('mediaUrl').focus();toast('Paste a URL then press Add source');return}
  if(source==='image'){ $('addImageBtn').click();return}
  if(source==='graphic'){ $('lowerName').focus();return}
  const item=await capture(source);
  if(!item)return;
  selected().layers=[{id:'layer-'+Date.now(),source,x:0,y:0,w:1,h:1,opacity:1,visible:true}];
  state.previewScene=state.selectedScene;
  render()
}
